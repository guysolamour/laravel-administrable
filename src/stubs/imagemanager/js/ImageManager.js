...
class ImageManager {
  ...
  setModalFooter() {
    ...
    this.getModalFooter().append(`
                ...
                <i class='fas fa-clock'></i> Dossiers: ${this.config[this.collection + '-label']} Fichiers: ${this.images.length} Taille fichiers: ${size}
                </div>
                `)
  }
  ...
  getPreviewedImageTemplate(image) {
    ...
    const html = `
            ...
            <a href="${image.url}" title='${this.isEmptyModel() ? image.new_name : image.name}' data-fancybox="gallery"
                ...
                <img style="height:${height}px; width:100%;" class="img-thumbnail" src='${this.isEmptyModel() ? image.url : image.thumb_sm_url}'
                data-id="${image.id}"
    ...
    if (this.isImagesCollection(this.collection)) {
      return `
                <div class="col-6 imagebox pb-2" data-id='${image.id}'>
  ...
  handleCloseIconClick(event) {
    ...
    swal({
      title: 'Suppression !',
      text: 'Etes de vous sûr de bien vouloir supprimer le média. Cette action est irréversible.',
      icon: 'warning',
      dangerMode: true,
      buttons: {
        cancel: 'Annulez',
